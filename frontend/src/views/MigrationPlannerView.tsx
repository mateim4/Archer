...
const MigrationPlannerView: React.FC = () => {
  ...
  const getMigrationOverview = () => [
    { 
      title: 'Total VMs', 
      value: selectedClusters.length > 0 ? 
        currentEnvironment?.clusters?.filter(c => selectedClusters.includes(c.id))
          .reduce((total, cluster) => total + (cluster.vms?.length || 0), 0) || 0 : 
        currentEnvironment?.clusters?.reduce((total, cluster) => total + (cluster.vms?.length || 0), 0) || 0, 
      icon: <DatabaseRegular />, 
      change: 'Available for migration',
      color: '#0066cc'
    },
    { 
      ...
      icon: <TargetRegular />, 
      change: `${currentEnvironment?.clusters?.length || 0} total clusters`,
      color: '#16a34a'
    },
    { 
      title: 'Migration Duration', 
      value: `${migrationDuration} months`, 
      icon: <SettingsRegular />, 
      change: `${migrationApproach} approach`,
      color: '#dc2626'
    },
    { 
      title: 'Migration Progress', 
      value: `Wave ${Math.ceil(migrationWave / 25)}`, 
      icon: <DiagramRegular />, 
      change: `${migrationWave}% per wave`,
      color: '#7c3aed'
    }
  ];
      ...
      case 'overview':
        return (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            {getMigrationOverview().map((stat, index) => (
              <div
                key={index}
                      ...
                      <div style={{
                        ...
                        <div className="grid grid-cols-2 gap-y-5 text-xs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '16px', alignItems: 'start' }}>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <span style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>vSphere: <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{cluster.vmwareVersion ? cluster.vmwareVersion.split(' ')[1] : 'Unknown'}</span></span>
                          </div>
                          <div className="flex items-start gap-3">
        ...
        return (
          <div>
            <div className="text-center py-12">
              <div className="mb-6">
                <p style={{ color: 'var(--text-secondary)' }}>Ready to generate your migration roadmap</p>
              </div>
              <PurpleGlassButton
                variant="primary"
                size="large"
      ...
      <div className="lcm-page-container">
        ...
        <div style={{ 
          ...
          {[
            ...
            { id: 'wizard', label: 'Migration Wizard', icon: <TargetRegular /> }
          ].map(tab => (
            <div
              key={tab.id}
