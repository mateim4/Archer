...
mod enhanced_rvtools_validation_tests {
    use crate::services::enhanced_rvtools_service::{ValidationRules, ValidationRuleType};
    use crate::models::project_models::RvToolsDataType;
    ...
    fn test_storage_capacity_validation() {
        ...
        for (value, should_be_valid, expected_type) in test_cases {
            ...
            assert_eq!(result.is_valid, should_be_valid, "Failed for value: {}", value);
            assert_eq!(result.detected_type, Some(expected_type), "Wrong type for value: {}", value);
        }
    }
    ...
    fn test_vsan_cluster_identification() {
        ...
        for cluster_name in vsan_indicators {
            ...
            assert!(result.is_valid, "Should process cluster name: {}", cluster_name);
        }
    }
    ...
    fn test_data_type_detection() {
        ...
        for (value, expected_type) in test_cases {
            ...
            assert_eq!(result.detected_type.unwrap(), expected_type, "Wrong type detection for: {}", value);
        }
    }
