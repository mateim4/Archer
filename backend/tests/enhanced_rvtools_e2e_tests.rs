...
mod enhanced_rvtools_e2e_tests {
    use crate::api::enhanced_rvtools::create_enhanced_rvtools_router;
    ...
    async fn setup_test_app() -> axum::Router {
        let db = Arc::new(Database::new_test().await.expect("Failed to create test database"));
        create_enhanced_rvtools_router(db)
    }
    ...
    async fn test_complete_workflow_simulation() {
        ...
        let file_data = create_mock_excel_file();
        let (body_data, content_type) = create_multipart_form_data("workflow-test.xlsx", &file_data);
        ...
        assert!(upload_response.status().is_client_error() || upload_response.status().is_server_error());
        ...
        let report_request = Request::builder()
            ...
            .uri("/generate-report")
            .header("content-type", "application/json")
            .body(Body::from(serde_json::to_vec(&report_request_body).unwrap()))
            .unwrap();
        ...
        let export_request = Request::builder()
            ...
            .uri("/export-report")
            .header("content-type", "application/json")
            .body(Body::from(serde_json::to_vec(&export_request_body).unwrap()))
            .unwrap();
    ...
    async fn test_confirmed_vsan_clusters_in_s2d_endpoint() {
        ...
        for cluster in confirmed_clusters {
            let request = Request::builder()
                .method(Method::GET)
                .uri(&format!("/s2d/compliance/test-upload?cluster_name={}", cluster))
                .body(Body::empty())
                .unwrap();

            let response = app.clone().oneshot(request).await.unwrap();
            assert_eq!(response.status(), StatusCode::OK, 
                "S2D compliance endpoint failed for cluster: {}", cluster);
            ...
            assert!(compliance_results.contains_key(cluster), 
                "Missing compliance result for cluster: {}", cluster);
        }
    }
    ...
    async fn test_api_response_structure() {
        ...
        for (endpoint, method) in endpoints_to_test {
            ...
            assert!(response_data.is_object(), "Response is not JSON object for: {}", endpoint);
            
            // Most responses should have timestamp fields
            if endpoint != "/validation/rules" {
                assert!(response_data.get("generated_at").is_some() || 
                       response_data.get("created_at").is_some(),
                       "Missing timestamp for: {}", endpoint);
            }
        }
    }
}