...
impl ProjectManagementService {
    ...
    pub async fn list_projects(
        ...
        for project_data in projects {
            ...
            let active_workflows = project_data
                .get("active_workflows")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32;
            let total_workflows = project_data
                .get("total_workflows")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32;
    ...
    pub async fn update_project(
        ...
        if let Some(progress_percentage) = request.progress_percentage {
            update_fields.insert(
                "progress_percentage",
                serde_json::to_value(progress_percentage)?,
            );
        }
    ...
    pub async fn delete_project(&self, project_id: &str) -> Result<()> {
        ...
        if !active_workflows.is_empty() {
            return Err(anyhow::anyhow!(
                "Cannot delete project with active workflows"
            ));
        }

        let _deleted: Option<Project> = self
            .db
    ...
    pub async fn create_workflow(
        ...
        let _project: Option<Project> = self.db.select(&project_thing).await?;
        ...
        let created: Vec<ProjectWorkflow> = self
            .db
    ...
    pub async fn list_project_workflows(&self, project_id: &str) -> Result<Vec<WorkflowResponse>> {
        ...
        for workflow_data in workflows {
            ...
            let total_activities = workflow_data
                .get("total_activities")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32;
            let completed_activities = workflow_data
    ...
    pub async fn list_available_hardware(
        &self,
        filter: Option<HardwareFilter>,
    ) -> Result<Vec<HardwarePoolResponse>> {
        ...
        let hardware: Vec<HardwarePool> = self
            .db
            .query(query)
            .await
        ...
        let responses = hardware
            .into_iter()
            .map(|hw| HardwarePoolResponse {
                id: hw.id.unwrap(),
    ...
    pub async fn allocate_hardware(
        &self,
        request: HardwareAllocationRequest,
        allocated_by: String,
    ) -> Result<Vec<HardwareAllocation>> {
        let project_thing = Thing::from(("project", request.project_id.as_str()));
        let workflow_thing = request
            .workflow_id
            .map(|id| Thing::from(("project_workflow", id.as_str())));
        ...
        for server_id in request.server_ids {
            ...
            let server: Option<HardwarePool> = self.db.select(&server_thing).await?;
    ...
    pub async fn hardware_pool_status(&self) -> Result<serde_json::Value> {
        ...
        for item in status {
            if let (Some(status), Some(count)) =
                (item.get("availability_status"), item.get("count"))
            {
                result.insert(
