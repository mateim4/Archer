...
use core_engine::models::hardware_basket::{
    CreateHardwareBasketRequest, HardwareBasket, HardwareConfiguration, HardwareModel,
    HardwarePricing, HardwareVendor,
};
...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route(
            "/hardware-baskets",
            get(get_hardware_baskets).post(create_hardware_basket),
        )
        ...
        .route(
            ...
            get(get_hardware_basket_models),
        )
        .route(
            "/hardware-baskets/:id/extensions",
            get(get_hardware_basket_extensions),
        )
        .route(
            "/hardware-configurations/:id",
            get(get_hardware_configuration),
        )
        .route(
            "/hardware-models/:id/specifications",
            axum::routing::put(update_hardware_model_specifications),
        )
        .route("/admin/cleanup", post(admin_cleanup))
        .route(
            "/hardware-baskets/:id/recount",
            post(recount_hardware_basket_totals),
        )
}
...
async fn recount_hardware_basket_totals(
    ...
    let basket_query =
        "SELECT * FROM hardware_basket WHERE id = type::thing('hardware_basket', $bid)";
    match db.query(basket_query).bind(("bid", &id)).await {
        Ok(mut res) => {
            ...
            if baskets.is_empty() {
                return (
                    ...
                    Json(ErrorResponse {
                        error: "Basket not found".to_string(),
                    }),
                )
                    .into_response();
            }
        ...
        Err(e) => {
            ...
            return (
                ...
                Json(ErrorResponse {
                    error: "Failed to verify basket".to_string(),
                }),
            )
                .into_response();
        }
    ...
    if let Ok(mut res) = db
        .query(
            "SELECT * FROM hardware_model WHERE basket_id = type::thing('hardware_basket', $bid)",
        )
        .bind(("bid", &id))
        .await
    {
...
async fn admin_cleanup(State(db): State<AppState>) -> impl IntoResponse {
    ...
    match db.query(query).await {
        Ok(_) => Json(CleanupResponse {
            success: true,
            message: "All hardware_* records deleted".to_string(),
        })
        .into_response(),
        Err(e) => {
...
async fn create_hardware_basket(
    ...
    match db
        .create::<Vec<HardwareBasket>>("hardware_basket")
        .content(basket)
        .await
    {
...
async fn upload_new_hardware_basket(
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        ...
        if name == "file" {
            ...
            let created_baskets: Vec<HardwareBasket> =
                match db.create("hardware_basket").content(basket).await {
                    Ok(baskets) => baskets,
                    Err(e) => {
            ...
            let _: Result<Vec<HardwareVendor>, _> =
                db.create("hardware_vendor").content(default_vendor).await;
            ...
            let (models, configurations, prices) =
                match parser.parse_file(&temp_path, &basket_id, &vendor_id) {
                    Ok(result) => {
                        tracing::info!(
                            "Successfully parsed file: {} models, {} configurations, {} prices",
                            result.0.len(),
                            result.1.len(),
                            result.2.len()
                        );
            ...
            let components_json: Vec<serde_json::Value> = configurations
                .iter()
                .map(|config| {
                    serde_json::json!({
            ...
            for (i, model) in models.iter().enumerate() {
                ...
                match db
                    .create::<Vec<HardwareModel>>("hardware_model")
                    .content(model.clone())
                    .await
                {
                    Ok(created) => {
                        ...
                        let created_json = created
                            ...
                            .unwrap_or_default();
                        tracing::debug!(
                            "Successfully saved model: {}, created_record: {}",
                            model.model_name,
                            created_json
                        );
                    }
            ...
            // Save configurations
            tracing::info!(
                "Attempting to save {} configurations to database",
                configurations.len()
            );
            for (i, config) in configurations.iter().enumerate() {
                ...
                match db
                    .create::<Vec<HardwareConfiguration>>("hardware_configuration")
                    ...
                    .await
                {
                    Ok(created) => {
                        ...
                        let created_json = created
                            ...
                            .unwrap_or_default();
                        tracing::debug!(
                            "Successfully saved configuration: {}, created_record: {}",
                            config.description,
                            created_json
                        );
                        ...
                        if let Some(created_rec) = created.first() {
                            ...
                            if let Some(ext_id) = created_rec.id.clone() {
                                ...
                                let existing_models: Option<HardwareModel> =
                                    match db.select(&model_id).await {
                                        Ok(m) => m,
                                        Err(e) => {
                                            tracing::error!(
                                ...
                                if let Some(existing) = existing_models.as_ref() {
                                    if let Some(exts) = &existing.extensions {
                                        for e in exts {
                                            merged_exts
                                                .push(serde_json::json!({"tb": e.tb, "id": e.id}));
                                        }
                                    }
                                }
                                merged_exts
                                    .push(serde_json::json!({"tb": ext_id.tb, "id": ext_id.id}));

                                let update_ext = serde_json::json!({"extensions": merged_exts});
                                tracing::info!(
                                    "Updating model {} with extension {} (merged total={})",
                                    model_record_str,
                                    serde_json::to_string(&ext_id).unwrap_or_default(),
                                    merged_exts.len()
                                );
                                tracing::info!(
                                    "Update payload: {}",
                                    serde_json::to_string(&update_ext).unwrap_or_default()
                                );

                                let res: Result<Option<HardwareModel>, _> =
                                    db.update(&model_id).merge(update_ext).await;
                                match res {
                                    Ok(updated_opt) => {
                                        let updated_count =
                                            if updated_opt.is_some() { 1 } else { 0 };
                                        tracing::info!(
                                            "Model update succeeded, updated count: {}",
                                            updated_count
                                        );
                                        let updated_json =
                                            serde_json::to_string(&updated_opt).unwrap_or_default();
                                        tracing::info!(
                                            "Model update returned JSON: {}",
                                            updated_json
                                        );

                                        match db
                                            .select::<Option<serde_json::Value>>(&model_id)
                                            .await
                                        {
                                            Ok(raw) => tracing::info!(
                                                "Post-merge DB select for {}: {}",
                                                model_record_str,
                                                serde_json::to_string(&raw).unwrap_or_default()
                                            ),
                                            Err(e) => tracing::error!(
                                                "Failed to re-select model {} after update: {}",
                                                model_record_str,
                                                e
                                            ),
                                        }
            ...
            for (i, price) in prices.iter().enumerate() {
                ...
                match db
                    .create::<Vec<HardwarePricing>>("hardware_pricing")
                    .content(price.clone())
                    .await
                {
                    Ok(_) => {
            ...
            let _: Vec<HardwareBasket> = db
                .update(&basket_record_id)
                .merge(update_data)
                .await
                .unwrap_or_default();

            tracing::info!(
                "Upload completed successfully: {} models, {} configurations, {} prices saved",
                models_saved,
                configs_saved,
                prices_saved
            );
            ...
            if models_saved > 0 {
                if let Ok(mut res) = db.query("SELECT * FROM hardware_model LIMIT 1").await {
                    ...
                    if let Some(mf) = fetched.get(0) {
                        tracing::info!(
                            "Debug fetched model from DB: {}",
                            serde_json::to_string(&mf).unwrap_or_default()
                        );
                    }
                }
            }
...
async fn upload_hardware_basket(
    ...
    let _: Vec<HardwareVendor> = db
        .create("hardware_vendor")
                        ...
                        if let Some(created) = created_configs.pop() {
                            ...
                            if let Some(ext_id) = created.id.clone() {
                                ...
                                if let Some(mid) = model_inner_id {
                                    ...
                                    match db.query(existing_query).bind(("mid", &mid)).await {
                                        Ok(mut res) => {
                                            ...
                                            if let Some(obj) = rows.get(0) {
                                                if let Some(exts) =
                                                    obj.get("extensions").and_then(|v| v.as_array())
                                                {
                                                    for e in exts {
                                                        merged_exts.push(e.clone());
                                                    }
                                                }
                                            }
                                        }
                                        Err(e) => tracing::error!(
                                            "Failed to fetch existing extensions for model {}: {}",
                                            mid,
                                            e
                                        ),
                                    }
                                    ...
                                    merged_exts.push(
                                        serde_json::json!({"tb": ext_id.tb, "id": ext_id.id}),
                                    );
...
async fn get_hardware_basket_models(
    ...
    let basket_query =
        "SELECT * FROM hardware_basket WHERE id = type::thing('hardware_basket', $basket_id)";
    ...
    let models_query =
        "SELECT * FROM hardware_model WHERE basket_id = type::thing('hardware_basket', $basket_id)";
    match db.query(models_query).bind(("basket_id", &id)).await {
        Ok(mut result) => {
            ...
            let mut price_map: std::collections::HashMap<String, (f64, String)> =
                    ...
                    Ok(mut pres) => {
                        ...
                        for p in rows {
                            ...
                            if let Some(mid) = p.model_id {
                                price_map.insert(
                                    format!("{}", mid),
                                    (p.net_price_usd, p.currency.clone()),
                                );
                            }
                        }
                    }
            ...
            let mut cfg_map: std::collections::HashMap<String, Vec<HardwareConfiguration>> =
                std::collections::HashMap::new();
            if !model_ids.is_empty() {
            ...
            let enriched: Vec<serde_json::Value> = models
                ...
                .map(|m| {
                    ...
                    let key = format!(
                        "{}",
                        m.id.clone().unwrap_or(Thing {
                            tb: "hardware_model".into(),
                            id: surrealdb::sql::Id::String(String::new())
                        })
                    );
                    if let Some(cfgs) = cfg_map.get(&key) {
                        // Processor
                        if specs.processor.is_none() {
                            if let Some(cpu_cfg) = cfgs.iter().find(|c| {
                                c.item_type.eq_ignore_ascii_case("processor")
                                    || c.description.to_lowercase().contains("cpu")
                                    || c.description.to_lowercase().contains("processor")
                            }) {
                                let text = format!(
                                    "{} {}",
                                    cpu_cfg.part_number.clone().unwrap_or_default(),
                                    cpu_cfg.description
                                );
                        ...
                        if specs.memory.is_none() {
                            if let Some(mem_cfg) = cfgs.iter().find(|c| {
                                c.item_type.eq_ignore_ascii_case("memory")
                                    || c.description.to_lowercase().contains("memory")
                                    || c.description.to_lowercase().contains("rdimm")
                                    || c.description.to_lowercase().contains("dimm")
                            }) {
                        ...
                        if specs.storage.is_none() {
                            if let Some(st_cfg) = cfgs.iter().find(|c| {
                                c.item_type.eq_ignore_ascii_case("storage")
                                    || c.description.to_lowercase().contains("ssd")
                                    || c.description.to_lowercase().contains("hdd")
                                    || c.description.to_lowercase().contains("nvme")
                            }) {
                        ...
                        if specs.network.is_none() {
                            if let Some(nc_cfg) = cfgs.iter().find(|c| {
                                c.item_type.eq_ignore_ascii_case("network")
                                    || c.description.to_lowercase().contains("nic")
                                    || c.description.to_lowercase().contains("ethernet")
                                    || c.description.to_lowercase().contains("gbe")
                            }) {
                    ...
                    v["base_specifications"] =
                        serde_json::to_value(&specs).unwrap_or(serde_json::json!({}));
                    ...
                    if let Some(processor_spec) = &specs.processor {
                        ...
                        if !processor_parts.is_empty() {
                            v["processor_info"] =
                                serde_json::Value::String(processor_parts.join(" "));
                        }
                    }
                    ...
                    if let Some(network_spec) = &specs.network {
                        ...
                        for port in &network_spec.ports {
                            network_parts
                                .push(format!("{}x {} {}", port.count, port.speed, port.port_type));
                        }
        ...
        Err(e) => {
            tracing::error!(
                "❌ Failed to fetch hardware models for basket {}: {}",
                id,
                e
            );
            (
    ...
    match db.query(query).bind(("cfg_id", &id)).await {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "Failed to fetch configuration".to_string(),
                }),
            )
                .into_response()
        }
...
fn derive_component_type(desc: &str, item_type: &str) -> String {
    ...
    if d.contains("retimer") {
        return "NVMe Retimer".to_string();
    }
    if d.contains("anybay") || d.contains("backplane") {
        return "JBOD".to_string();
    }
    if d.contains("riser") {
        return "Riser".to_string();
    }
    if d.contains("heatsink") {
        return "Heatsink".to_string();
    }
    if d.contains("power supply") || d.contains("psu") {
        return "PSU".to_string();
    }
    if d.contains("chassis") {
        return "Chassis".to_string();
    }
    if d.contains("nic") || d.contains("ethernet") || d.contains("lan") {
        return "NIC".to_string();
    }
    if d.contains("ssd") || d.contains("hdd") {
        return "Drive".to_string();
    }
    if d.contains("dimm") || d.contains("memory") || d.contains("rdimm") {
        return "DIMM".to_string();
    }
    if d.contains("processor") || d.contains("cpu") {
...
fn derive_size(desc: &str) -> Option<String> {
    ...
    if let Some(re) = &re_ports {
        if let Some(cap) = re.captures(d) {
            if let Some(m) = cap.get(1) {
                return Some(format!("{}-Port", m.as_str()));
            }
...
async fn get_hardware_basket_extensions(
    ...
    if let Ok(mut res) = db.query(models_query).bind(("basket_id", &id)).await {
        ...
        for v in rows {
            if let Some(s) = v.get("id") {
                if let Ok(t) = serde_json::from_value::<Thing>(s.clone()) {
                    model_ids.push(t);
                }
            }
        }
    ...
    match db.query(cfg_query).bind(("model_ids", &model_ids)).await {
        Ok(mut res) => {
            configs = res.take(0).unwrap_or_default();
        }
        Err(e) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "Failed to fetch extensions".to_string(),
                }),
            )
                .into_response();
        }
        ...
        if let Ok(mut res) = db.query(p_query).bind(("cfg_ids", &cfg_ids)).await {
            ...
            for v in rows {
                let cid = v
                    ...
                    .and_then(|x| serde_json::from_value::<Thing>(x.clone()).ok());
    ...
    // Build enriched rows
    let rows: Vec<ExtensionRow> = configs
        .into_iter()
        .map(|c| {
            ...
            let price = c.id.as_ref().and_then(|cid| {
                price_map
                    .get(&format!("{}", cid))
                    .map(|(a, cur)| serde_json::json!({"amount": a, "currency": cur}))
            });
            ExtensionRow {
                id: c.id.clone(),
...
async fn delete_hardware_basket(
    ...
    if !model_ids.is_empty() {
        ...
        if let Ok(mut res) = db.query(cfg_query).bind(("model_ids", &model_ids)).await {
            ...
            for row in cfgs {
                if let Some(cid) = row.get("id") {
                    config_ids.push(cid.clone());
                }
            }
        }
    }
    ...
    let _ = db
        .query(
            r#"DELETE hardware_model WHERE basket_id = type::thing('hardware_basket', $basket_id)"#,
        )
        .bind(("basket_id", &id))
        .await;
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: format!("Failed to delete basket: {}", e),
                }),
            )
                .into_response()
        }
