...
impl HardwareBasketParser {
    ...
    fn parse_dell_vendor_config(&self, workbook: &mut Xlsx<std::io::BufReader<std::fs::File>>) -> Result<ParsedVendorConfig> {
    ...
    fn parse_lenovo_vendor_config(&self, workbook: &mut Xlsx<std::io::BufReader<std::fs::File>>) -> Result<ParsedVendorConfig> {
        // Parse basic vendor config - can be enhanced later
        let mut exchange_rates = HashMap::new();
    ...
    fn parse_dell_lot_pricing(&self, worksheet: &Range<DataType>, file_path: &str) -> (Vec<ParsedHardwareLot>, Vec<ParsedHardwareComponent>, Vec<String>) {
        ...
        for row_idx in (header_row + 1)..rows {
            let row = worksheet.get_row(row_idx);
            ...
            if let Some(cell_0) = row.get(0) {
                if let Some(lot_desc) = cell_0.as_string() {
                    if lot_desc.contains("SMI") || lot_desc.contains("SMA") {
                        ...
                        let list_price = row.get(3).and_then(|c| c.as_float());
                        let net_price_usd = row.get(4).and_then(|c| c.as_float());
                        let net_price_eur = row.get(5).and_then(|c| c.as_float());
                        let price_3yr_ps = row.get(6).and_then(|c| c.as_float());
                        let price_5yr_ps = row.get(7).and_then(|c| c.as_float());
                        let price_3yr_psp = row.get(8).and_then(|c| c.as_float());
                        let price_5yr_psp = row.get(9).and_then(|c| c.as_float());
                    ...
                    if let Some(item_type) = item_type_cell.as_string() {
                        if !item_type.trim().is_empty() && item_type != "Server" {
                            ...
                            let component = ParsedHardwareComponent {
                                ...
                                quantity: 1,
                                unit_price_usd: row.get(4).and_then(|c| c.as_float()),
                                unit_price_eur: row.get(5).and_then(|c| c.as_float()),
                                total_price_usd: row.get(4).and_then(|c| c.as_float()),
                                total_price_eur: row.get(5).and_then(|c| c.as_float()),
    ...
    fn parse_lenovo_server_lots(&self, worksheet: &Range<DataType>, file_path: &str) -> (Vec<ParsedHardwareLot>, Vec<ParsedHardwareComponent>, Vec<String>) {
        ...
        for row_idx in (header_row + 2)..rows { // Start from row 5 (skip empty row 4)
            let row = worksheet.get_row(row_idx);
            if row.is_none() { continue; }
            let row = row.unwrap();
                        ...
                        if let Some(desc_cell) = row.get(2) {
                            if let Some(description) = desc_cell.as_string() {
                                if description.contains("SMI") {
                                    ...
                                    let price_1yr_usd = row.get(4).and_then(|c| c.as_float());
                                    let price_1yr_eur = row.get(5).and_then(|c| c.as_float());
                                    let price_3yr_usd = row.get(6).and_then(|c| c.as_float());
                                    let price_3yr_eur = row.get(7).and_then(|c| c.as_float());
                                    
                                    let lot = ParsedHardwareLot {
                                        vendor: "Lenovo".to_string(),
                                        lot_code: lot_code.to_string(),
                                        lot_description: description,
                                        base_part_number: Some(part_number),
                ...
                if let Some(part_cell) = row.get(1) {
                    if let Some(part_number) = part_cell.as_string() {
                        if !part_number.trim().is_empty() {
                            if let Some(desc_cell) = row.get(2) {
                                if let Some(description) = desc_cell.as_string() {
                                    let quantity = row.get(3).and_then(|c| c.as_float()).unwrap_or(1.0) as i32;
                ...
                if let Some(part_number) = part_cell.as_string() {
                    if !part_number.trim().is_empty() {
                        ...
                        let option = ParsedHardwareOption {
                            ...
                            unit_price_usd: row.get(2).and_then(|c| c.as_float()),
                            unit_price_eur: row.get(3).and_then(|c| c.as_float()),
                            currency: "USD".to_string(),
                            specifications: serde_json::json!({}),
                            excel_source_file: file_path.to_string(),
